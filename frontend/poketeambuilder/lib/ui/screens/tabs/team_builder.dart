...
class TeamBuilder extends StatefulWidget {
  ...
  TeamBuilder(
      {Key? key, required this.currentTrainer, this.teamToEdit, this.editMode})
      : super(key: key);
  ...
  Future<void> _fetchAllPokemonNames() async {
    try {
      List<String> allPokemonNames =
          await _pokeAPIService.fetchAllPokemonNames();
      setState(() {
  ...
  Future<void> _updateCurrentGeneration(String generation) async {
    ...
    if (generation == 'All generations') {
      ...
      List<String> pokemonNames =
          await _pokeAPIService.fetchPokemonNamesByGeneration(generationNumber);
      setState(() {
    ...
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        if (widget.editMode == true) {
          return AlertDialog(
            title: Text('Team successfully edited'),
            actions: <Widget>[
              TextButton(
                onPressed: () {
                        ...
                        showDialog(
                          ...
                          builder: (BuildContext context) {
                            return AlertDialog(
                              ...
                              content: Text(
                                  'You cannot save an empty team. Please add at least one Pokémon.'),
                              actions: <Widget>[
                                TextButton(
                                  onPressed: () {
                                    Navigator.of(context).pop();
